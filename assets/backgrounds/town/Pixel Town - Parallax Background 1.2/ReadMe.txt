Copyright (c) 2022 Digital Moons
https://digitalmoons.itch.io/

Customer support email: digitalmoonsstudio@gmail.com
Discord: @Digitalmoons#3448

--------------------------------
By buying this background pack you have a permission to use the content of it as a part of your personal or commercial project, for example as a background for your game, website or app. You're not allowed to share, re-sell or redistribute this background pack or any assets/backgrounds/images from it outside from your project in any way or on any platform. You aren't allowed to re-sell or alter and re-sell the images, or otherwise attempt to claim the work as your own.


Link to the asset pack:
https://digitalmoons.itch.io/pixel-town
--------------------------------


Pixel Town - Pixel art Background pack by Digital Moons 


Content:

- Includes 5 versions of the background:
    Day, Night, Sunset, Winter day, Winter night

- Each background includes 9 or 10 layers

- Background resolution is 240x135px and all images are in .png format


Rooftop Backgrounds content:

- This background pack includes rooftop version of each background
- Each background includes 5 layers

You can use content of this asset pack as a part of your commercial or personal projects, no crediting needed.
 
--------------------------------
 
If you have any suggestions about how to improve this pack, please leave a comment below the asset on itch.io

Thank you for buying this background pack! 
- Digital Moons